...
printf '%s\n' \
  ...
  '' \
  '## This release: live frame-generation switching, Adaptive stability, and SteamOS recovery' \
  '' \
  ...
  '- **Live frame-generation switch:** The first control in **Frame Generation Mode** now turns generation on or off immediately, without changing the selected Fixed or Adaptive settings. Turn it back on to resume with the same profile; this replaces the old request to overload the multiplier with an “Off” value.' \
  '- **Adaptive Frame Generation:** Optional 30–240 FPS targeting with a configurable 2x–4x ceiling; fixed 2x, 3x, and 4x remain the default. Adaptive mode now defaults to a **90 FPS** target with **Smooth Cadence** enabled. Smooth Cadence is game- and hardware-dependent: it can improve both smoothness and perceived responsiveness, but can be turned off when a game feels better without it. Live Adaptive changes briefly reset timing and stability calculations, so allow a few seconds for output to settle before judging it.' \
  ...
  '- **2x short-hitch recovery:** Once Adaptive has validated its 2x ceiling, an isolated 100–250 ms gameplay hitch keeps that proven level, refreshes three real history frames, and resumes instead of taking the longer menu/focus recovery path. Fixed mode and Adaptive 3x/4x are unchanged.' \
  '- **Profile compatibility settings:** Launcher-only options such as Disable LSFG-VK on Next Launch, Base FPS Cap, WoW64, Steam Deck Mode, and Zink persist with the selected Decky profile. Existing wrapper settings are imported automatically during the upgrade.' \
  '- **Isolation cleanup:** Removed Gamescope WSI and MangoHud controls. The isolated launcher replaces the normal global implicit-layer search path with its private one, so those controls cannot take effect for experimental-wrapper games.' \
  '- **Per-game profiles:** Create a named profile in Decky, set **Active In** to the game executable, and lsfg-vk selects its engine settings automatically. Returning the UI selector to Default does not disable matching profiles.' \
